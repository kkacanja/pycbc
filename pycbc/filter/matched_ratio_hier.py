...
from pycbc.filter.matched_ratio_opttime import RatioMatchedFilterControl
...
#        print(f"self.snr_threshold{self.snr_threshold}")
    ...
    def process_segment_coarse(self, stilde, psd, ref_template, filters_f,
        ...
        self._reset_kernel_timers()

        f_high_idx = int(self.f_high / psd.delta_f) + 1

        stilde_c = stilde[:f_high_idx]
        psd_c    = psd[:f_high_idx]
        tmpl_c   = ref_template[:f_high_idx]
        ...
        snr, _, norm = matched_filter_core(
            tmpl_c, stilde_c, psd_c,
            low_frequency_cutoff=ref_template.f_lower,
            high_frequency_cutoff=self.f_high,
            h_norm=h_norm
        )
        t2 = time.time()
        logging.info(f"COARSE:matched_filter_core time {t2-t1:.6f}")

        decimate = int(np.round(self.tap_sr / self.engine_sr))

        self.ref_snr = snr.numpy() * (norm * stilde_c.delta_t) / decimate
        ...
        coarse_n_taps = n_taps
        t1 = time.time()
        local_idxs, t_idxs, snr_vals = self._execute_blocked_kernel(
                self.ref_snr, filters_f, coarse_n_taps,
                valid_slice=coarse_valid_slice if windows is None else None,
                windows=windows
            )
        t2 = time.time()
        logging.info(f"COARSE:_execute_blocked_kernel time {t2-t1:.6f}")

        # self._log_kernel_timers(label="coarse") # Uncomment if method defined
    ...
    def process_segment_fine(self, stilde, psd, ref_template, filters_f,
        ...
        t2 = time.time()
        logging.info(f"FINE:matched_filter_core time {t2-t1:.6f}")

        self.ref_snr = snr.numpy() * (norm * stilde.delta_t)
        t1 = time.time()
        local_idxs, t_idxs, snr_vals = \
            self._execute_blocked_kernel(
                self.ref_snr, filters_f, n_taps,
                valid_slice=valid_slice if windows is None else None,
                windows=windows
            )
        t2 = time.time()
        logging.info(f"FINE:_execute_blocked_kernel time {t2-t1:.6f}")
